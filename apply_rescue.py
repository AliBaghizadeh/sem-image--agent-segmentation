"""
Script to apply targeted enhancement and MatSAM rescue to failed mask cases.
Can process either specific failed cases (from data/selected_tiled_masks) 
or all images in the dataset.

Usage:
    # Rescue only failed cases using best optimized parameters
    python apply_rescue.py

    # Re-process all images using a specific blend/intensity
    python apply_rescue.py --all --blend 0.5
"""
import os
import sys
import torch
...
def apply_rescue(args):
    ...
    for base_name in files_to_process:
        ...
        # Optional Overlay: Original image | Binary mask (side-by-side)
        if args.overlay:
            # Convert mask to RGB for consistent stacking
            mask_rgb = cv2.cvtColor(final_mask, cv2.COLOR_GRAY2RGB)
            
            # Create side-by-side: original (left) | mask (right)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blend", type=float, default=0.0, help="Blend ratio (1.0 = raw, 0.0 = full enhancement)")
